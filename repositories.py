...
ORDER_ACTIVE_CLIENT_STATUSES = ("new", "offered", "matched", "in_progress")
ORDER_ACTIVE_MASTER_STATUSES = ("matched", "in_progress")
ORDER_OPEN_FOR_OFFERS_STATUSES = ("new", "offered")
MASTER_ADMIN_STATUSES = ("approved", "blocked")
...
ALLOWED_MASTER_STATUSES = {"pending", "approved", "blocked"}
ALLOWED_AVAILABILITY = {"online", "offline"}
...
async def refresh_master_online_statuses():
    ts = now_ts()
    threshold = ts - settings.online_timeout
    await execute(
        """
        UPDATE masters
        SET availability='offline', updated_at=$1
...
async def touch_master_presence(user_id: int):
    ...
    await execute(
        """
        UPDATE masters
        SET last_seen=$1, availability='online', updated_at=$1
...
async def client_active_orders_count(user_id: int) -> int:
    row = await fetchrow(
        ...
        user_id,
        list(ORDER_ACTIVE_CLIENT_STATUSES),
    )
...
async def master_active_orders_count(user_id: int) -> int:
    row = await fetchrow(
        ...
        user_id,
        list(ORDER_ACTIVE_MASTER_STATUSES),
    )
    return int(row["c"]) if row else 0
...
async def create_offer(
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            existing_offer = await conn.fetchrow(
                """
                SELECT id
                FROM offers
                WHERE order_id=$1 AND master_user_id=$2 AND status='active'
                LIMIT 1
                """,
                order_id,
                master_user_id,
            ...
            if not order_row or order_row["status"] not in ORDER_OPEN_FOR_OFFERS_STATUSES:
                return None
            ...
            await conn.execute(
                """
                UPDATE orders
                SET status='offered', updated_at=$1
                WHERE id=$2 AND status IN ('new', 'offered')
                """,
                ts,
                order_id,
            )
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            offer = await conn.fetchrow(
                ...
                SELECT
                    offers.*,
                    orders.user_id AS client_user_id,
            ...
            if offer["order_status"] not in ("new", "offered"):
                return None
...
async def list_new_orders_for_master(category: str):
    return await fetch(
        ...
        SELECT *
        FROM orders
        WHERE status = ANY($1::text[]) AND category=$2
        ORDER BY id DESC
        """,
        list(ORDER_OPEN_FOR_OFFERS_STATUSES),
        category,
    )
...
async def list_active_orders_for_master(user_id: int):
    return await fetch(
        ...
        SELECT *
        FROM orders
        WHERE selected_master_id=$1 AND status = ANY($2::text[])
        ORDER BY id DESC
        """,
        user_id,
        list(ORDER_ACTIVE_MASTER_STATUSES),
...
async def create_chat_message(
    ...
    await execute(
        ...
        INSERT INTO chat_messages(
            chat_id, order_id, sender_user_id, sender_role, message_type, text, file_id, created_at
        )
    ...
    await execute("UPDATE chats SET updated_at=$1 WHERE id=$2", ts, chat_id)
...
async def close_chat(order_id: int):
    await execute(
        "UPDATE chats SET status='closed', updated_at=$1 WHERE order_id=$2",
        now_ts(),
        order_id,
    )
...
async def cancel_order(order_id: int):
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute(
                "UPDATE offers SET status='rejected' WHERE order_id=$1 AND status IN ('active', 'selected')",
                order_id,
            )
            await conn.execute(
                "UPDATE chats SET status='closed', updated_at=$1 WHERE order_id=$2",
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            await conn.execute(
                """
                UPDATE orders
                SET selected_master_id=NULL, status='offered', updated_at=$1
                WHERE id=$2
                """,
                ts,
                order_id,
            ...
            await conn.execute(
                ...
                UPDATE offers
                SET status='rejected'
                WHERE order_id=$1 AND status='selected'
                """,
                order_id,
            )
...
async def set_order_status(order_id: int, status: str, selected_master_id: Optional[int] = None):
...
async def update_master_profile(user_id: int, field_name: str, value):
    if field_name not in ALLOWED_MASTER_PROFILE_FIELDS:
        raise ValueError(f"Updating field '{field_name}' is not allowed")
...
async def set_master_status_by_id(master_id: int, status: str, availability: Optional[str] = None):
    if status not in ALLOWED_MASTER_STATUSES:
        raise ValueError(f"Invalid master status: {status}")
    ...
    if availability is not None:
        ...
        await execute(
            "UPDATE masters SET status=$1, availability=$2, updated_at=$3 WHERE id=$4",
            status,
            availability,
            ts,
            master_id,
        ...
        await execute(
            "UPDATE masters SET status=$1, updated_at=$2 WHERE id=$3",
            status,
            ts,
            master_id,
        )
...
async def admin_stats():
    ...
    for key, query in keys.items():
        ...
        data[key] = int(row["c"]) if row else 0
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            chat = await conn.fetchrow(
                "SELECT * FROM chats WHERE order_id=$1 FOR UPDATE",
                order_id,
            )
            if chat:
                return chat
...
async def save_chat_message(chat_id: int, sender_role: str, text: str):
    ...
    sender_user_id = (
        chat["client_user_id"] if sender_role == "client" else chat["master_user_id"]
    )
