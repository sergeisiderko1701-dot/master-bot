...
from repositories import create_chat_message, fetchrow, get_chat_for_order, get_chat_history
...
def register(dp):
    ...
    async def client_chat(call: types.CallbackQuery, state: FSMContext):
        ...
        order = await fetchrow(
            '''
            SELECT * FROM orders
            WHERE id=$1 AND user_id=$2 AND selected_master_id IS NOT NULL AND status = ANY($3::text[])
            ''',
        ...
        await state.update_data(chat_role="client", order_id=order_id, target_user_id=chat["master_user_id"], chat_id=chat["id"])
        await ChatFlow.message.set()
        await call.message.answer(chat_open_text(order_id, True), reply_markup=chat_reply_kb())
        await call.answer()
    ...
    async def master_chat(call: types.CallbackQuery, state: FSMContext):
        ...
        await state.update_data(chat_role="master", order_id=order_id, target_user_id=chat["client_user_id"], chat_id=chat["id"])
        await ChatFlow.message.set()
        await call.message.answer(chat_open_text(order_id, False), reply_markup=chat_reply_kb())
        await call.answer()
    ...
    async def close_chat_reply(message: types.Message, state: FSMContext):
        await state.finish()
        await message.answer("💬 <b>Чат закрито</b>\n\nПовертаю вас у головне меню.", reply_markup=main_menu_kb(is_admin_user=is_admin(message.from_user.id)))
    ...
    async def relay_chat(message: types.Message, state: FSMContext):
        ...
        try:
            if message.photo:
                ...
                await create_chat_message(chat_id, order_id, message.from_user.id, role, "photo", caption, message.photo[-1].file_id)
                await dp.bot.send_photo(target_id, message.photo[-1].file_id, caption=chat_media_caption(order_id, role, caption, "📷"), reply_markup=exit_chat_inline())
                await message.answer("📷 <b>Фото надіслано</b>", reply_markup=chat_reply_kb())
                return

            if message.video:
                caption = normalize_text(message.caption or "", 1000)
                await create_chat_message(chat_id, order_id, message.from_user.id, role, "video", caption, message.video.file_id)
                await dp.bot.send_video(target_id, message.video.file_id, caption=chat_media_caption(order_id, role, caption, "📹"), reply_markup=exit_chat_inline())
                await message.answer("📹 <b>Відео надіслано</b>", reply_markup=chat_reply_kb())
                return

            text = normalize_text(message.text or "", 1500)
            if not text:
                await message.answer("Напишіть повідомлення текстом або надішліть фото / відео.", reply_markup=chat_reply_kb())
                return

            await create_chat_message(chat_id, order_id, message.from_user.id, role, "text", text, None)
            await dp.bot.send_message(target_id, chat_text_message(order_id, role, text), reply_markup=exit_chat_inline())
            await message.answer("✅ <b>Повідомлення надіслано</b>", reply_markup=chat_reply_kb())
        except Exception as e:
            await message.answer(f"Не вдалося переслати повідомлення: {e}", reply_markup=chat_reply_kb())
