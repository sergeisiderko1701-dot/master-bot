...
def client_order_actions_inline(order_id: int, status: str):
    ...
    if status in ["matched", "in_progress"]:
        ...
        kb.add(InlineKeyboardButton("📜 Історія чату", callback_data=f"chat_history_{order_id}"))
...
def selected_order_master_actions(order_id: int):
    ...
    kb.add(
        ...
        InlineKeyboardButton("📜 Історія чату", callback_data=f"chat_history_{order_id}"),
...
def chat_reply_kb():
    kb = ReplyKeyboardMarkup(resize_keyboard=True)
    kb.add(KeyboardButton("📜 Історія"), KeyboardButton("❌ Закрити чат"))
    kb.add(KeyboardButton("⬅️ Назад"), KeyboardButton("🏠 У меню"))
    return kb
