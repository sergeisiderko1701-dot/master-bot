...
async def send_order_card(bot: Bot, chat_id: int, order_row, title="📦 <b>Заявка</b>", reply_markup=None):
    master_name = await get_master_name(order_row["selected_master_id"])
    text = order_card_text(order_row, title, master_name)
    if order_row["media_type"] == "photo" and order_row["media_file_id"]:
        await bot.send_photo(chat_id, order_row["media_file_id"], caption=text, reply_markup=reply_markup)
    elif order_row["media_type"] == "video" and order_row["media_file_id"]:
...
async def send_admin_order_detail(bot: Bot, chat_id: int, order, offers):
    ...
    if order["media_type"] == "photo" and order["media_file_id"]:
        await bot.send_photo(chat_id, order["media_file_id"], caption=detail_text, reply_markup=admin_order_actions_inline(order["id"], order["status"]))
    elif order["media_type"] == "video" and order["media_file_id"]:
        await bot.send_video(chat_id, order["media_file_id"], caption=detail_text, reply_markup=admin_order_actions_inline(order["id"], order["status"]))
    else:
        await bot.send_message(chat_id, detail_text, reply_markup=admin_order_actions_inline(order["id"], order["status"]))


async def notify_masters_about_order(bot: Bot, order_row, masters):
    for master in masters:
        try:
            await send_order_card(bot, master["user_id"], order_row, title="🔔 <b>Нова заявка</b>", reply_markup=order_card_master_actions(order_row["id"]))
            await bot.send_message(master["user_id"], "Натисніть <b>📨 Відгукнутись</b>, якщо хочете взяти цю заявку в роботу.")
