...
def welcome_text() -> str:
    return (
        "🔧 <b>Одеса Майстер</b>\n\n"
        "Сервіс для швидкого пошуку майстра в Одесі.\n\n"
        "🚀 Швидко • Зручно • Без дзвінків\n\n"
        "👤 <b>Клієнт</b> — створює заявку та отримує пропозиції\n"
        "🔧 <b>Майстер</b> — приймає заявки та знаходить клієнтів\n\n"
...
def tip_reopen_order() -> str:
    return (
        ...
        "Ви можете:\n"
        "• натиснути <b>🔄 Майстер не відповідає</b>\n"
        "• повторно відкрити заявку\n"
        "• обрати іншого майстра або дочекатись нових пропозицій"
    )
...
def order_reopened_text(order_id: int) -> str:
    return (
        ...
        "Тепер ви можете обрати іншого майстра або дочекатися нових пропозицій."
    )
...
def no_offers_yet_text(order_id: int) -> str:
    return (
        ...
        "Ми вже показали її відповідним майстрам.\n"
        "Зазвичай перші відповіді приходять протягом <b>5–15 хв</b>.\n\n"
        "Щоб отримати відповідь швидше:\n"
        "• додайте фото або відео проблеми, якщо ще не додали\n"
        "• опишіть проблему конкретніше\n"
        "• вкажіть точний район або орієнтир\n\n"
        "Можете трохи зачекати або скасувати заявку, якщо вона вже неактуальна."
    )


def offers_available_nudge_text(order_id: int, offers_count: int) -> str:
    return (
        f"📬 <b>По заявці #{order_id} вже є пропозиції</b>\n\n"
        f"Доступних пропозицій: <b>{offers_count}</b>.\n"
        "Перегляньте їх і оберіть майстра, який підходить найкраще."
    )
