...
from ui_texts import ask_district_text, master_profile_text
...
def register(dp):
    @dp.message_handler(lambda m: m.text == "🔧 Майстер", state="*")
    async def master_entry(message: types.Message, state: FSMContext):
        ...
        if master and master["status"] == "blocked":
            await message.answer("Ваш профіль майстра заблокований. Зверніться в підтримку.", reply_markup=main_menu_kb(is_admin_user=is_admin(message.from_user.id)))
            return

        await MasterRegistration.name.set()
        await message.answer("👤 <b>Реєстрація майстра</b>\n\nВведіть ваше ім'я:", reply_markup=back_menu_kb())
    ...
    async def reg_district(message: types.Message, state: FSMContext):
        ...
        await MasterRegistration.next()
        await message.answer("🧾 <b>Коротко про себе</b>\n\nНапишіть кілька речень про ваш досвід і спеціалізацію.", reply_markup=back_menu_kb())
    ...
    async def reg_description(message: types.Message, state: FSMContext):
        ...
        await MasterRegistration.next()
        await message.answer("🛠 <b>Досвід роботи</b>\n\nНапишіть, з чим саме допомагаєте клієнтам.", reply_markup=back_menu_kb())
    ...
    async def reg_experience(message: types.Message, state: FSMContext):
        ...
        await MasterRegistration.next()
        await message.answer("📞 <b>Контактний телефон</b>\n\nВведіть номер у зручному форматі.", reply_markup=back_menu_kb())

    @dp.message_handler(state=MasterRegistration.phone)
    async def reg_phone(message: types.Message, state: FSMContext):
        await state.update_data(phone=normalize_text(message.text, 50))
        await MasterRegistration.next()
        await message.answer("📸 <b>Фото профілю</b>\n\nНадішліть фото або напишіть <b>пропустити</b>.", reply_markup=back_menu_kb())
    ...
    async def reg_photo(message: types.Message, state: FSMContext):
        ...
        await state.finish()
        await message.answer("⏳ <b>Заявку надіслано</b>\n\nПісля перевірки адміністратор активує ваш профіль.", reply_markup=main_menu_kb(is_admin_user=is_admin(message.from_user.id)))

    @dp.message_handler(lambda m: m.text == "👤 Мій профіль", state="*")
    async def profile(message: types.Message, state: FSMContext):
        master = await approved_master_row(message.from_user.id)
        if not master:
            await message.answer("Профіль майстра недоступний.", reply_markup=main_menu_kb(is_admin_user=is_admin(message.from_user.id)))
            return
        await show_master_profile(message, master)
    ...
    async def new_orders(message: types.Message, state: FSMContext):
        master = await approved_master_row(message.from_user.id)
        if not master:
            await message.answer("❌ Ви не підтверджений майстер.", reply_markup=main_menu_kb(is_admin_user=is_admin(message.from_user.id)))
            return
        ...
        for row in rows:
            await send_order_card(dp.bot, message.chat.id, row, title="📢 Доступна заявка", reply_markup=order_card_master_actions(row["id"]))
    ...
    async def active_orders(message: types.Message, state: FSMContext):
        ...
        for row in rows:
            await send_order_card(dp.bot, message.chat.id, row, title="📄 Активна заявка", reply_markup=selected_order_master_actions(row["id"]))
