...
def register(dp):
    ...
    async def menu_handler(message: types.Message, state: FSMContext):
        await state.finish()
        await message.answer(menu_text(), reply_markup=main_menu_kb(is_admin_user=is_admin(message.from_user.id)))

    @dp.callback_query_handler(Text(equals="exit_chat"), state="*")
    async def exit_chat(call: types.CallbackQuery, state: FSMContext):
        await state.finish()
        await call.message.answer("💬 <b>Чат завершено</b>\n\nПовертаю вас у головне меню.", reply_markup=main_menu_kb(is_admin_user=is_admin(call.from_user.id)))
        await call.answer()
    ...
    async def support_start(message: types.Message, state: FSMContext):
        ...
        await message.answer(support_intro())
